...
- It does not run tests. That's `/goal`'s job (and `protocols/run_goal_tests.md` if the user has installed the verification protocol).
- It does not generate per-user-story goal folders or test-plan files. Use the full prompt kit at `art-of-vibe-coding/_canvas/codex-goal/prompts/` for that.
- It does not work for non-WordPress projects.
- It does not fix bugs. For bug fixes, use the bug-fix templates in the user's `codex-goal-templates.md` directly.
